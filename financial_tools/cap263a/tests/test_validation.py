...
def test_accuracy_floor():
    r = accuracy_report()
    # current: raw ~78.4%, +defensible ~82.4%, high-conf precision ~84.0%, review ~35%
    assert r["raw_agreement"] >= 0.74, f"raw agreement regressed to {r['raw_agreement']:.1%}"
    assert r["high_conf_precision"] >= 0.79, \
