...
def test_accuracy_floor():
    r = accuracy_report()
    # current: raw ~77.6%, +defensible ~81.6%, high-conf precision ~82.8%, review ~35%
    assert r["raw_agreement"] >= 0.73, f"raw agreement regressed to {r['raw_agreement']:.1%}"
    assert r["high_conf_precision"] >= 0.78, \
