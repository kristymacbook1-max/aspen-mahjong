...
- **12 hardening-pass corrections (§5):** found across two independent multi-agent audit rounds (initial + adversarial re-verification), all fixed and regression-tested (54 tests, up from 37); the SSCM labor-ratio fix (#16) changes computed dollar output on any TB with significant Excluded-tier labor (sales/R&D compensation) and should be re-run against any output generated before this pass.
