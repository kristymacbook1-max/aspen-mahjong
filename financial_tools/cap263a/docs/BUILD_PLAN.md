...
**STATUS UPDATE 2026-07-09 (BUILD EXECUTED) — the engines this plan specifies are now IMPLEMENTED CODE, see `docs/TAX_DECISIONS.md` §15.** Five parallel build agents + a foundation/integration pass shipped: multi-format ingestion (`readers.py` — xlsx workbook/csv/json/directory), the materialized tax-basis TB (`engines/tax_basis_tb.py`), MSPM/SRM (`engines/inventory.py`, dispatched from `compute_unicap`), SCA (`engines/sca.py` + `taxonomy/sca_drivers.yaml`), §263A(f) (`engines/interest.py`), §174/§174A (`engines/re_capitalization.py`), §1.263(a)-4/-5 + §195/§248/§709 + §280B (`engines/intangibles.py`), §59(e) (`engines/qualified_expenditures.py` + periods YAML), §1060 (`engines/purchase_price_allocation.py`), the shared basis-reconciliation helper, the Phase E interview (`interview.py` + `taxonomy/interview.yaml`, 119 nodes, Gates 0-10), and the end-to-end orchestrator (`pipeline.run_engagement`). Every golden worked example in this plan passes as an automated test — MSPM $284,400/$3,284,400 (including end-to-end from interview answers through the dispatcher), SRM $36,875, SCA $55k/$55k, §263A(f) $376,428.57 — suite 201 passing. Still spec-only: the report-tab extensions for the new engines (results surface via the `run_engagement` dict; workbook tabs remain the existing five), Phase H's classifier taxonomy codes, and everything in "Deferred / out of scope."
